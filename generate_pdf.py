...
LEVEL_COLORS = {
    1: colors.HexColor("#2E7D32"),
    2: colors.HexColor("#F57F17"),
    3: colors.HexColor("#E65100"),
    4: colors.HexColor("#B71C1C"),
}
LEVEL_BG_COLORS = {
...
LEVEL_TEXT = {1: "LEVEL 1", 2: "LEVEL 2", 3: "LEVEL 3", 4: "LEVEL 4"}
RISK_TEXT  = {1: "Low Risk", 2: "Moderate Risk", 3: "High Risk", 4: "Extreme Risk"}
...
def level_badge_table(level):
    ...
    badge = Table(
        [[Paragraph(f'<font size="9"><b>{LEVEL_TEXT.get(level,"N/A")}</b></font>',
                    ParagraphStyle("b", fontName="Helvetica-Bold", fontSize=9, alignment=TA_CENTER,
                                   textColor=lvl_color)),
          Paragraph(f'<font size="9">{RISK_TEXT.get(level,"Unknown")}</font>',
...
def build_country_row(country_data, styles):
    ...
    content = [Paragraph(name, styles["country_name"]), Spacer(1, 3)]
    if summary:
        content.append(Paragraph(summary, styles["body_small"]))
        content.append(Spacer(1, 4))
    if security:
        content.append(Paragraph(f'<b>Security:</b> {security}', styles["body_small"]))
    if health:
        content.append(Paragraph(f'<b>Health:</b> {health}', styles["body_small"]))
    if crime:
        content.append(Paragraph(f'<b>Crime:</b> {crime}', styles["body_small"]))

    row_table = Table([[level_badge_table(level), content]], colWidths=[54*mm, 126*mm])
...
def build_legend(styles):
    items = [
        Paragraph(
            f'<font color="{LEVEL_COLORS[l].hexval()}"><b>{LEVEL_TEXT[l]}</b></font> — {RISK_TEXT[l]}',
            ParagraphStyle("li", fontName="Helvetica", fontSize=9,
                           textColor=colors.HexColor("#333333"))
        )
        for l in [1,2,3,4]
    ]
    t = Table([items], colWidths=[45*mm]*4)
...
def generate_pdf(classified_data, output_path="travel_risk_report.pdf"):
    ...
    # Header — each element on its own line, no overlap
    story.append(Paragraph("Across Assist", styles["title"]))
    story.append(Paragraph("Travel Risk Advisory Report", styles["subtitle"]))
    story.append(Paragraph(f"Country Risk Classification · Last updated: {date_str}", styles["updated"]))
    ...
    for c in classified_data:
        counts[c.get("level", 2)] = counts.get(c.get("level", 2), 0) + 1

    stats_data = [[
        Paragraph(
            f'<font size="18" color="{LEVEL_COLORS[l].hexval()}"><b>{counts[l]}</b></font><br/>'
    ...
    regions = {}
    for c in classified_data:
        regions.setdefault(c.get("region","Other"), []).append(c)
    ...
    for region, countries in regions.items():
        ...
        for country in countries:
            story.append(KeepTogether([build_country_row(country, styles), Spacer(1, 5)]))
        story.append(Spacer(1, 8))
